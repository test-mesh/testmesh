...
import (
	...
	gitpkg "github.com/test-mesh/testmesh/internal/git"
...
// CreateFixPR creates a fix branch and pull request from a high-confidence suggestion.
func (e *SelfHealingEngine) CreateFixPR(
	ctx context.Context,
	suggestion *models.Suggestion,
	gitProvider gitpkg.GitProvider,
	repoName string,
	baseBranch string,
	baseSHA string,
) (prURL string, err error) {
	if suggestion.SuggestedYAML == "" {
		return "", fmt.Errorf("suggestion has no suggested YAML")
	}

	branchName := fmt.Sprintf("testmesh/fix/%s", suggestion.ID.String()[:8])

	if err := gitProvider.CreateBranch(ctx, repoName, branchName, baseSHA); err != nil {
		return "", fmt.Errorf("create branch: %w", err)
	}

	flowPath := fmt.Sprintf("flows/%s.yaml", suggestion.FlowID.String()[:8])

	commitMsg := fmt.Sprintf("fix: %s\n\nAutomatic fix by TestMesh self-healing.\nConfidence: %.0f%%\nSuggestion: %s",
		suggestion.Title, suggestion.Confidence*100, suggestion.ID.String())

	changes := []gitpkg.FileChange{{
		Path:    flowPath,
		Content: suggestion.SuggestedYAML,
	}}

	if _, err := gitProvider.PushFileChanges(ctx, repoName, branchName, commitMsg, changes); err != nil {
		return "", fmt.Errorf("push changes: %w", err)
	}

	prBody := fmt.Sprintf("## TestMesh Auto-Fix\n\n**Suggestion:** %s\n\n**Description:** %s\n\n**Reasoning:** %s\n\n**Confidence:** %.0f%%\n\n---\n🤖 Generated by TestMesh self-healing engine",
		suggestion.Title, suggestion.Description, suggestion.Reasoning, suggestion.Confidence*100)

	pr, err := gitProvider.CreatePullRequest(ctx, repoName, gitpkg.PullRequestCreate{
		Title: fmt.Sprintf("[TestMesh] %s", suggestion.Title),
		Body:  prBody,
		Head:  branchName,
		Base:  baseBranch,
	})
