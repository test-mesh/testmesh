...
<Tabs items={["Homebrew", "Windows", "curl", "npm", "Build from Source"]}>
  <Tab value="Homebrew">
    ```bash
...
If you use Claude Code, install the TestMesh plugin to get the MCP server and 8 built-in skills:

```bash
/plugin install testmesh
```

This gives you:
- **MCP server** — Claude Code can analyze your services, generate flows, run tests, and check coverage without leaving the IDE
- **Skills** — `generate-flow`, `run-flow`, `validate-flow`, `action-patterns`, `debugging-flows`, `flow-layers`, `mcp-tools`, `yaml-schema`

<Callout type="info">
  The plugin requires the TestMesh CLI to be installed and on your `PATH`. Install it using one of the options above first.
</Callout>

For manual MCP configuration (VS Code, Cursor, or other MCP clients), see [MCP Integration](/docs/features/mcp-integration#using-with-claude-code).
