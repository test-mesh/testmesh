...
Create a **Source Control** integration in TestMesh pointing at your Argo CD instance. Store the Argo CD API token as an encrypted secret.
...
data:
  template.testmesh-sync-success: |
    webhook:
      testmesh:
        method: POST
        path: /api/v1/integrations/webhooks/argocd
        body: |
          {
...
TestMesh receives the webhook at `POST /api/v1/integrations/webhooks/argocd`, looks up the matching `GitTriggerRule` by `app_name`, and fires the configured Suite.
...
Create a **Source Control** integration pointing at Gitea so TestMesh can write commit statuses.
