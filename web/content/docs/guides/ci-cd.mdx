...
If you use Argo CD to deploy to Kubernetes, TestMesh can trigger suite runs automatically when an Application syncs — without any changes to your GitHub Actions or GitLab CI pipelines.

<Steps>
  <Step>
    Configure the Argo CD integration and a GitTriggerRule in TestMesh pointing to the Suite you want to run.
  </Step>
  <Step>
    Add an `argocd-notifications` webhook that POSTs to `POST /api/v1/integrations/webhooks/argocd` after each successful sync.
  </Step>
  <Step>
    TestMesh runs the Suite and posts the result back to Gitea as a commit status.
  </Step>
</Steps>

See the [GitOps Integration](/docs/guides/gitops-integration) guide for the full setup, including warm environment reuse and the Service Under Test pattern.
...
<Cards>
  ...
  <Card title="GitOps Integration" href="/docs/guides/gitops-integration" description="Trigger suites automatically from Argo CD syncs." />
