...
The TestMesh plugin is available in the Claude Code plugin registry. Install it with one command:

```bash
/plugin install testmesh
```

This installs the MCP server and 8 skills (`generate-flow`, `run-flow`, `validate-flow`, `action-patterns`, `debugging-flows`, `flow-layers`, `mcp-tools`, `yaml-schema`) that guide you through common workflows.
