...
export default function SuiteEditPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  useEffect(() => {
    if (suite && !initialized) {
      ...
      setFlowEntries(
        (suite.flows ?? []).sort((a, b) => a.order - b.order).map((sf) => ({
          flow_id: sf.flow_id,
  ...
  const availableFlows = flowsData?.flows?.filter(
    (f) => f.name.toLowerCase().includes(flowSearch.toLowerCase()) && !flowEntries.some((e) => e.flow_id === f.id)
  ) ?? [];
    ...
    return (
      <div className="px-6 py-6">
        <div className="rounded-xl bg-[#0f1923] border border-[#1e2d3d] p-6">
          ...
          <Link href="/suites" className="inline-flex items-center gap-1.5 h-7 px-3 rounded-lg text-xs text-[#7fa8c8] bg-[#0b0f18] border border-[#1e2d3d] hover:border-[#2a3d52] transition-colors">
            Back to Suites
          </Link>
        </div>
      </div>
    ...
    <div className="px-6 py-6 max-w-3xl">
      <div className="flex items-center gap-2 mb-5">
        ...
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-xl font-semibold text-[#c8dce8]">Edit Suite</h1>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="rounded-xl bg-[#0f1923] border border-[#1e2d3d] overflow-hidden">
          <div className="px-4 py-2.5 border-b border-[#1a2332]">
            ...
            <span className="text-[10px] text-[#4a6480] ml-2">configure which flows run and in what order</span>
          </div>
              ...
              <div className="space-y-2">
                ...
                {sortedEntries.map((entry) => (
                  <div key={entry.flow_id} className="flex items-center gap-2 p-2.5 rounded-lg border border-[#1e2d3d] bg-[#0b0f18]">
                    ...
                    <button
                      type="button" onClick={() => removeFlow(entry.flow_id)}
