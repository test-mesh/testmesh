...
import { useCreateSchedule, useCronPresets, useTimezones, useValidateCron } from '@/lib/hooks/useSchedules';
  ...
  return (
    <div className="px-6 py-6 max-w-3xl">
      <div className="flex items-center gap-2 mb-5">
        ...
        <h1 className="text-xl font-semibold text-[#c8dce8]">Create Schedule</h1>
        <p className="text-xs text-[#3d5670] mt-0.5">Set up automated test runs on a recurring schedule</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex items-center gap-1.5 text-xs">
                {validateCron.data?.valid ? (
                  <>
                    ...
                    <span className="text-teal-400">{describeCron(cronExpr)}</span>
                  </>
                ) : validateCron.data?.error ? (
                  <>
                    <XCircle className="h-3.5 w-3.5 text-red-400" />
                    <span className="text-red-400">{validateCron.data.error}</span>
                  </>
                ) : null}
              </div>
            </div>
        ...
        <div className="flex justify-end gap-2">
          ...
          <button
            type="submit"
            disabled={createMutation.isPending || !name || !flowId || !validateCron.data?.valid}
          ...
          >
            {createMutation.isPending ? <><RefreshCw className="h-3 w-3 animate-spin" />Creating…</> : 'Create Schedule'}
          </button>
        </div>
      </form>
    </div>
